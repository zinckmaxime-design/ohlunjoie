...
// Client Supabase (anti-conflit: ne pas utiliser le nom "supabase")
const sb = (window.supabase && typeof window.supabase.createClient === 'function')
  ? window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY)
  : null;

if (!sb) {
  console.error('❌ Supabase non chargé (CDN). Vérifie le <script> supabase-js dans index.html.');
}
...
async function trackPageView() {
  if (!sb) return;
  await sb.from('page_analytics').insert({
    page_name: 'public',
...
$('#admin-form').addEventListener('submit', async (e) => {
  ...
  if (!sb) { toast('❌ Supabase non disponible'); return; }
  const { data: admin } = await sb.from('admins').select('*').eq('email', email).eq('is_active', true).single();
  ...
  if (admin && admin.password_hash === pass) {
    ...
    const { data: perms } = await sb.from('admin_roles').select('*').eq('admin_id', admin.id);
    adminPermissions = {};
    perms.forEach(p => {
...
async function loadAdminDashboard() {
  ...
  const [inscRes, eventsRes, pageViewsRes, emailsRes] = await Promise.all([
    ...
    sb.from('events').select('id', { count: 'exact', head: true }).eq('visible', true).eq('archived', false),
    sb.from('page_analytics').select('id', { count: 'exact', head: true }),
    sb.from('inscriptions').select('email')
  ]);
  
  const { data: allEvents } = await sb.from('events').select('id, max_participants, inscriptions(count)').order('date');
...
async function loadAdminEvents() {
  ...
  const { data: events } = await sb.from('events').select('*').order('date', { ascending: false });
  ...
  for (const ev of events) {
    const { count } = await sb.from('inscriptions').select('id', { count: 'exact', head: true }).eq('event_id', ev.id);
    html += `<tr>
      <td>${ev.titre}</td>
      <td>${ev.date}</td>
      <td>${ev.lieu}</td>
...
async function loadAdminInscriptions() {
  ...
  const { data: events } = await sb.from('events').select('id, titre').order('date', { ascending: false });
...
async function filterInscriptions() {
  ...
  let query = sb.from('inscriptions').select('*');
...
async function loadAdminVolunteers() {
  ...
  async function renderList() {
    ...
    const { data: events } = await sb.from('events').select('id, date').gte('date', year + '-01-01').lte('date', year + '-12-31');
    const { data: inscs } = await sb.from('inscriptions').select('*');
...
async function adminEditUser(id) {
  ...
  const { data: droits } = await sb.from('admin_roles').select('*').eq('admin_id', id);
...
async function adminDeleteUser(id) {
  ...
  await sb.from('admin_roles').delete().eq('admin_id', id);
...
if (adminFormElement) {
  adminFormElement.onsubmit = async function(e) {
    ...
    try {
      if (!sb) { toast('❌ Supabase non disponible'); return; }
      if (!id) {
        const { data: created, error } = await sb.from('admins').insert(adminData).select().single();
        ...
        const { error: updateError } = await sb.from('admins').update(adminData).eq('id', id);
...
async function loadAdminUsers() {
  ...
  const { data: admins } = await sb.from('admins').select('*').order('created_at');
...
async function initSiteConfig() {
  if (!sb) return;
  const { data: configs } = await sb.from('site_config').select('id').limit(1);
  if (!configs || configs.length === 0) {
    await sb.from('site_config').insert([{
...
async function loadAdminAssociation() {
  ...
  const { data: config } = await sb.from('site_config').select('*').limit(1).single();
...
async function saveAssociationConfig() {
  ...
  try {
    if (config) {
      await sb.from('site_config').update(updateData).eq('id', config.id);
    } else {
      await sb.from('site_config').insert([updateData]);
    }
...
function adminDeleteEvent(id) {
  ...
  sb.from('events').delete().eq('id', id).then(() => {
    toast('✅ Supprimé');
    loadAdminEvents();
  });
}
...
if (editFormElement) {
  editFormElement.onsubmit = async function(e) {
    ...
    await sb.from('events').update({ titre, date, lieu }).eq('id', id);
    toast('✅ Modifié');
    modal.closeAll();
    loadAdminEvents();
  };
...
async function fetchPublicEvents() {
  if (!sb) return [];
  const { data } = await sb.from('events').select('*').eq('visible', true).eq('archived', false).order('date', { ascending: true });
  return data || [];
}
